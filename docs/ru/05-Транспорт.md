...
```cpp
struct UdpConfig {
    QString localAddress   = QStringLiteral("0.0.0.0");  // хост/IP строкой
    quint16 localPort      = 0;            // 0 — любой свободный
    QString remoteAddress;                 // адрес узла назначения
    quint16 remotePort     = 0;
    bool    bindBeforeSend = true;         // привязать сокет при open()
};
```

> Адреса — обычные строки (а не `QHostAddress`), чтобы ядро библиотеки не зависело от QtNetwork; конкретный UDP-транспорт разбирает их на своей стороне.
