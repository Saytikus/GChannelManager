...
| `incomingRequests` | `DecodedMessage::Type::Request` — запрос от узла к нам |
| `cachedRepliesResent` | повторный `Request` с тем же `corrId` обслужен из кэша ответов (см. [Кэш ответов](06-Gateway-API.md#кэш-ответов-серверная-роль)) |
| `sessionStartsSent` | отправлен `SessionStart` (`startSession()`) |
| `sessionStartsReceived` | получен `SessionStart` от узла |
| `sessionStartTimeouts` | не дождались `SessionStartAck` за `sessionStartTimeout` |
| `sessionStopsSent` | отправлен `SessionStop` (`stopSession()`) |
| `sessionStopsReceived` | получен `SessionStop` от узла |

> [!NOTE] Что не считается
...
См. подробности — [Тестирование](09-Тестирование.md).
