...
Подробнее про переходы — [[03-Состояния-и-переходы#Канал]].
...
Машина состояний — [[03-Состояния-и-переходы#Сессия]].
...
`setKeepAliveConfig` применяется **на лету**: если сессия запущена, изменение `enabled`/`interval` подхватывается немедленно. См. таблицу матрицы переходов в [[03-Состояния-и-переходы#Включение/выключение keep-alive на лету|разделе keep-alive]].
...
Поля `GatewayStats` и список инкрементов — отдельная страница: [[07-Статистика]].
